...
const Testimonials = () => {
  return (
    <section className="flex flex-col items-center w-full h-full gap-5 py-10 bg-black/20">
      <h1 className="text-4xl font-bold lg:text-5xl">Success Stories</h1>
      ...
      <div className="flex flex-col justify-between gap-6 px-5 mt-5 lg:flex-row lg:w-4/5">
        {cards.map((items, index) => (
          <div
            key={index}
            className="flex flex-col flex-1 gap-4 p-5 bg-white/10 rounded-2xl"
          >
            ...
            <StarRating rating={5} />
            <p className="text-justify text-gray-400">{items.description}</p>
          </div>
        ))}
      </div>
    </section>
